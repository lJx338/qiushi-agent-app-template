...
图文步骤见 [离线 HTML 教程](tutorial.html)，双击即可打开，无需联网或启动服务。命令和交付边界见[平台接入指南](PLATFORM-INTEGRATION.md)，验证级别见[本地模拟与真实环境对照](LOCAL-AND-REAL-ENVIRONMENTS.md)，失败时按[制品提交与失败排错](SUBMISSION-TROUBLESHOOTING.md)留存最小复现。

实现前先阅读仓库内的应用最佳实践说明；它规定 PRD、领域层、平台端口、场景页面、行为测试和交付证据的顺序。报价应用是实践样例，业务规则不可直接复制。

开发检查点：先记录设计确认，再记录领域/schema 摘要；遇到公共契约缺口或重复失败，在本应用仓库的唯一 Linear Issue Workpad 记录命令、最短错误、已尝试方案、最小提案和影响。未使用 Linear 时，使用本应用的 PR 或 Issue 记录同样信息。完成固定 SHA、evidence 和验证后在该 PR/Issue 交付。不要直播所有过程，也不要用 mock 或测试常量代替运行时行为；所有模拟与真实环境结论按 `LOCAL-AND-REAL-ENVIRONMENTS.md` 标注。
...
3. 使用 @qiushi/app-kit 定义动作；业务代码放 `src/app.ts`，`src/host.ts` 导出 Edge runtime extension。DSH 只可通过显式可选 compatibility profile 接入，不是默认 Host 或强制 peer。
